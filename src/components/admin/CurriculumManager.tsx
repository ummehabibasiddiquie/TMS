...
export function CurriculumManager() {
  ...
  function inferDayType(day: CurriculumDay): "CHECKLIST" | "TRAINING" | "MIXED" {
    const hasChecklist = day.checklistItems.some(
      (i) => (i.kind || "CHECKLIST") !== "WORK"
    );
    const hasWork = day.checklistItems.some((i) => i.kind === "WORK");
    const hasLessons = day.lessons.length > 0;
    const hasTraining = hasWork || hasLessons;
    if (hasChecklist && hasTraining) return "MIXED";
  ...
  async function saveDay() {
    if (!canEdit) return;
    setMsg("");
    const dayType = selected ? inferDayType(selected) : "MIXED";
  ...
  async function saveChecklistItem() {
    if (!canEdit || !editingItemId) return;
    if (!editItemForm.title.trim()) {
      setMsg("Title is required");
      return;
    }
    setSavingItem(true);
    const res = await fetch(`/api/curriculum/checklist/${editingItemId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    setSavingItem(false);
    if (!res.ok) {
      setMsg(data.error || "Failed to update item");
      return;
    }
  ...
  async function deleteChecklistItem(itemId: string) {
    if (!canEdit) return;
    if (editingItemId === itemId) cancelEditChecklistItem();
                            ...
                            {editingItemId === item.id ? (
                              <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <button
                                    type="button"
                                    onClick={() => void saveChecklistItem()}
                                    disabled={savingItem}
                                    className="inline-flex items-center gap-1 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium hover:bg-blue-500 disabled:opacity-50"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="inline-flex items-center gap-1 rounded-lg border border-slate-600 px-3 py-1.5 text-xs hover:bg-slate-800"
                                  >
                    ...
                    <div>
                      ...
                      <ul className="mb-4 space-y-2">
                        {workOnly.map((item) => (
                          <li
                            key={item.id}
                            className="rounded-xl border border-amber-900/40 bg-amber-950/20 px-3 py-2"
                          >
                            {editingItemId === item.id ? (
                              <div className="space-y-2">
                                <input
                                ...
                                <div className="flex flex-wrap gap-2">
                                  <button
                                    ...
                                    onClick={() => void saveChecklistItem()}
                                    disabled={savingItem}
                                    className="inline-flex items-center gap-1 rounded-lg bg-amber-600 px-3 py-1.5 text-xs font-medium hover:bg-amber-500 disabled:opacity-50"
                                  >
                                    <Save className="h-3.5 w-3.5" />
                                    {savingItem ? "Saving..." : "Save"}
                                  </button>
